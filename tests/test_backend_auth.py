"""Tests for auth bootstrap, deployment auth flow, and CORS behavior."""
...
def test_deployed_web_login_me_refresh_logout_flow():
    def runner(store_path: Path, base_dir: Path):
        ...
        with patch.dict(os.environ, env, clear=False):
            ...
            with TestClient(app) as client:
                ...
                login = client.post('/api/auth/login', json={'username': 'owner', 'password': 'secret-pass-123'})
                me = client.get('/api/auth/me')
                protected = client.get('/api/sites')
                refreshed = client.post('/api/auth/refresh')
                logout = client.post('/api/auth/logout')
                me_after_logout = client.get('/api/auth/me')
                protected_after_logout = client.get('/api/sites')
        ...
        assert state.runtime_settings.access_cookie_name in login.cookies
        assert state.runtime_settings.refresh_cookie_name in login.cookies
        ...
        assert refreshed.json()['username'] == 'owner'
